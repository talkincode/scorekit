...
Texture recordings may be WAV, FLAC, OGG, or any other format the installed
FFmpeg can decode. Before placement, FFmpeg converts them to stereo 16-bit PCM
at the build's `--sample-rate`; scorekit does not implement resampling or
channel conversion. The normalized intermediates are command-scoped and are
