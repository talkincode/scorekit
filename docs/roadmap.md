...
- **音乐语法引擎（Music Grammar lint，M6 首选候选，2026-07 创作实战复盘产出）**：把"审美"变成可校验断言。`scorekit lint scene.yaml --grammar grief.yaml`——语法档案是外部数据文件，声明一组确定性约束（tempo 范围、旋律留白占比、同时旋律声部上限、最短乐句时长、终止完全性、允许的和声进行等），编译器只校验、不生成一个音符，与"不内嵌作曲智能"铁律相容。价值：Agent 获得**审美回归测试**——风格体系沉淀为数据，跨模型换代存续；创作反馈回路从"人耳听"变为"机器断言 + 人耳终审"。可行性已验证：候选约束全部可从 ScoreIr 确定性计算（留白比 = 休止拍/总拍；终止完全性 = 末乐句是否落 i/1 度）。
